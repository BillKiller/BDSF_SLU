...
def get_semantic_bad_case(sentents, correct_intents, pred_intents, correct_slots, slot_outputs, bio_list=None):
    ...
    for t, p in zip(correct_slots, slot_outputs):
        ...
        for j in range(len(t)):
            if p[j] != t[j]:
                ...
                break
        index += 1
    for (idx, value) in enumerate(semantic_acc):
        if value == False:
            bad_sentences.append(sentents[idx])
            bad_correct_intents.append(correct_intents[idx])
            bad_pred_intents.append(pred_intents[idx])
            bad_correct_slots.append(correct_slots[idx])
            bad_pred_slots.append(slot_outputs[idx])
            if bio_list is not None and  idx < len(bio_list):
                bad_bio_list.append(bio_list[idx])
    return bad_sentences, bad_correct_intents, bad_pred_intents, bad_correct_slots, bad_pred_slots, bad_bio_list


def write_to_human(name, input_sentences, correct_intent, pred_intents, correct_slots, pred_slots, bio_lists):
    with open(name, 'w') as f:
        if len(bio_lists) == 0:
            for (input_sentence_item, correct_intent_item, pred_intent_item, correct_slot_item, pred_slot_item) in zip(
                    input_sentences, correct_intent, pred_intents, correct_slots, pred_slots):
                for word, c_slot, p_slot in zip(input_sentence_item, correct_slot_item, pred_slot_item):
                    f.write("{} {} {}\n".format(word, c_slot, p_slot))
                f.write("{}<=>{}\n".format(
                    correct_intent_item, pred_intent_item))
                f.write('\n')
        else:
            for (input_sentence_item, correct_intent_item, pred_intent_item, correct_slot_item, pred_slot_item, bio) in zip(
                    input_sentences, correct_intent, pred_intents, correct_slots, pred_slots, bio_lists):
                for word, c_slot, p_slot, p_bio in zip(input_sentence_item, correct_slot_item, pred_slot_item, bio):
                    f.write("{} {} {} {}\n".format(
                        word, c_slot, p_slot, p_bio))
                f.write("{}<=>{}\n".format(
                    correct_intent_item, pred_intent_item))
                f.write('\n')

# def write_to_human(name, input_sentences, correct_intent, pred_intents, correct_slots, pred_slots):
#     with open(name, 'w') as f:
#         for (input_sentence_item, correct_intent_item, pred_intent_item, correct_slot_item, pred_slot_item) in zip(
#                 input_sentences, correct_intent, pred_intents, correct_slots, pred_slots):
#             f.write("{} <=>{}<=>{}\n".format(
#                 "".join(input_sentence_item), correct_intent_item, pred_intent_item))
#             f.write(" ".join(correct_slot_item) + '\n')
#             f.write(" ".join(pred_slot_item) + '\n')
...
class Processor(object):
    ...
    def train(self):
        ...
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            for text_batch, slot_batch, intent_batch, bio_batch in tqdm(dataloader, ncols=50):
                padded_text, [sorted_slot, sorted_intent, sorted_bio], seq_lens = self.__dataset.add_padding(
                    text_batch, [(slot_batch, False),
                                 (intent_batch, False), (bio_batch, False)]
                )
                sorted_intent = [item * num for item,
                                 num in zip(sorted_intent, seq_lens)]
                sorted_intent = list(Evaluator.expand_list(sorted_intent))

                text_var = Variable(torch.LongTensor(padded_text))
                slot_var = Variable(torch.LongTensor(
                    list(Evaluator.expand_list(sorted_slot))))
                bio_var = Variable(torch.LongTensor(
                    list(Evaluator.expand_list(sorted_bio))))
                ...
                force_intent = None
                force_bio = None
                random_slot, random_intent, random_bio = random.random(
                ...
                if random_intent < self.__dataset.intent_forcing_rate:
                    force_intent = intent_var
                if random_bio < self.__dataset.bio_forcing_rate:
                    force_bio = bio_var

                slot_out, intent_out, bio_out = self.__model(
                    text_var, seq_lens, forced_slot=slot_var, forced_intent=intent_var, force_bio=force_bio
                )
                ...
                batch_loss = slot_loss + intent_loss + bio_loss
            ...
            time_con = time.time() - time_start
            self.logger.info('[Epoch {:2d}]: The total slot loss on train data is {:2.6f}, intent data is {:2.6f}, bio data is {:2.6f}, cost '
            ...
            dev_f1_score, dev_acc, dev_sent_acc, _ = self.estimate(
            ...
            if dev_f1_score > best_dev_slot or dev_acc > best_dev_intent or dev_sent_acc > best_dev_sent:
                test_f1, test_acc, test_sent_acc, bio_f1 = self.estimate(
                ...
                self.logger.info('\nTest result: slot f1 score: {:.6f}, intent acc score: {:.6f}, semantic '
                      'accuracy score: {:.6f}. BIO f1 score {:.6f} '.format(test_f1, test_acc, test_sent_acc, bio_f1))

                model_save_dir = os.path.join(self.__dataset.save_dir, "model")
                if not os.path.exists(model_save_dir):
                    os.mkdir(model_save_dir)

                torch.save(self.__model, os.path.join(
                    model_save_dir, "model.pkl"))
                torch.save(self.__dataset, os.path.join(
                    model_save_dir, 'dataset.pkl'))

                time_con = time.time() - time_start
                self.logger.info('[Epoch {:2d}]: In validation process, the slot f1 score is {:2.6f}, '
    ...
    def estimate(self, if_dev, test_batch=100):
        ...
        if if_dev:
            pred_slot, real_slot, pred_intent, real_intent, _, _, pred_bio = self.prediction(
        ...
        bio = []
        for s in real_slot:
            bio.append([i[0] for i in s])
        bio_f1_socre = miulab.computeF1Score(bio, pred_bio)[0]
        slot_f1_socre = miulab.computeF1Score(pred_slot, real_slot)[0]
        intent_acc = Evaluator.accuracy(pred_intent, real_intent)
        sent_acc = Evaluator.semantic_acc(
            pred_slot, real_slot, pred_intent, real_intent)

        return slot_f1_socre, intent_acc, sent_acc, bio_f1_socre

    @staticmethod
    def validate(model_path, dataset_path, batch_size, index=0):
        """
        ...
        pred_slot, real_slot, exp_pred_intent, real_intent, pred_intent, text, pred_bio = Processor.prediction(
            model, dataset, "test", batch_size
        )
        write_to_human('result{}.txt'.format(index), text,
                       real_intent, exp_pred_intent, real_slot, pred_slot, pred_bio)

        bad_sentences, bad_correct_intents, bad_pred_intents, bad_correct_slots, bad_pred_slots, bad_bio_list = get_semantic_bad_case(text,
                       real_intent, exp_pred_intent, real_slot, pred_slot, bio_list = pred_bio)
        write_to_human('bad_result{}.txt'.format(index),
                       bad_sentences, bad_correct_intents,bad_pred_intents, bad_correct_slots, bad_pred_slots,  pred_bio)
        
        # To make sure the directory for save error predict ion.
        ...
        sent_acc = Evaluator.semantic_acc(
            pred_slot, real_slot, exp_pred_intent, real_intent)
    ...
    def prediction(model, dataset, mode, batch_size):
        ...
        if mode == "dev":
            dataloader = dataset.batch_delivery(
                'dev', batch_size=batch_size, shuffle=False, is_digital=False)
        elif mode == "test":
            dataloader = dataset.batch_delivery(
                'test', batch_size=batch_size, shuffle=False, is_digital=False)
        else:
            raise Exception(
        ...
        text = []
        for text_batch, slot_batch, intent_batch, bio_batch in tqdm(dataloader, ncols=50):
            padded_text, [sorted_slot, sorted_intent, sorted_text, sorted_bio], seq_lens = dataset.add_padding(
                text_batch, [(slot_batch, False), (intent_batch, False), (text_batch, False), (bio_batch, False)], digital=False
            )
            ...
            slot_idx, intent_idx, bio_idx = model(
                var_text, seq_lens, n_predicts=1)
            nested_slot = Evaluator.nested_list(
                [list(Evaluator.expand_list(slot_idx))], seq_lens)[0]
            nested_bio = Evaluator.nested_list(
                [list(Evaluator.expand_list(bio_idx))], seq_lens)[0]
            pred_slot.extend(dataset.slot_alphabet.get_instance(nested_slot))
            pred_bio.extend(dataset.bio_alphabet.get_instance(nested_bio))
        ...
        return pred_slot, real_slot, exp_pred_intent, real_intent, pred_intent, text, pred_bio
    ...
    def nested_list(items, seq_lens):
        ...
        for jdx in range(0, len(seq_lens)):
            for idx in range(0, num_items):
                trans_items[idx].append(
                    items[idx][count:count + seq_lens[jdx]])
            count += seq_lens[jdx]
