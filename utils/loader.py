...
from utils.data_utils import bio2bmes
...
class Alphabet(object):
    ...
    def __init__(self, name, if_use_pad, if_use_unk, if_span=False):
        ...
        if if_span:
            self.__sign_span = '<ENT>'
            self.add_instance(self.__sign_span)
...
class TorchDataset(Dataset):
    ...
    def __init__(self, text, slot, intent, bio):
        self.__text = text
        self.__slot = slot
    ...
    def __getitem__(self, index):
        return self.__text[index], self.__slot[index], self.__intent[index], self.__bio[index]

    def __len__(self):
        # Pre-check to avoid bug.
        assert len(self.__text) == len(self.__slot)
        assert len(self.__text) == len(self.__intent)
        assert len(self.__bio) == len(self.__text)
...
class DatasetManager(object):
    ...
    def __init__(self, args):
        ...
        if not args.use_bert:
            self.__word_alphabet = Alphabet(
                'word', if_use_pad=True, if_use_unk=True, if_span=True)
        else:
            self.__word_alphabet = Alphabet(
                'word', if_use_pad=False, if_use_unk=False, if_span=False)
            
        self.__slot_alphabet = Alphabet(
            'slot', if_use_pad=False, if_use_unk=False)
        self.__intent_alphabet = Alphabet(
            'intent', if_use_pad=False, if_use_unk=False)
        self.__bio_alphabet = Alphabet(
            'bio', if_use_pad=False, if_use_unk=False)
    ...
    @property
    def bio_alphabet(self):
        return deepcopy(self.__bio_alphabet)
    ...
    @property
    def bio_forcing_rate(self):
        return self.__args.bio_forcing_rate
    ...
    def show_summary(self):
        ...
        print('\tnumber of train sample:                    {};'.format(
            len(self.__text_word_data['train'])))
        print('\tnumber of dev sample:                      {};'.format(
            len(self.__text_word_data['dev'])))
        print('\tnumber of test sample:                     {};'.format(
            len(self.__text_word_data['test'])))
        ...
        print('\trate of dropout in network:                {};'.format(
            self.__args.dropout_rate))
        print('\tteacher forcing rate(slot)		    		{};'.format(
            self.slot_forcing_rate))
        print('\tteacher forcing rate(intent):		    	{};'.format(
            self.intent_forcing_rate))
    ...
    def quick_build(self):
        ...
        self.__bio_alphabet.save_content(alphabet_dir)
    ...
    def get_dataset(self, data_name, is_digital):
        ...
        if is_digital:
            return self.__digit_word_data[data_name], \
                ...
                self.__digit_bio_data[data_name]
        else:
            return self.__text_word_data[data_name], \
                self.__text_slot_data[data_name], \
                self.__text_intent_data[data_name],\
                self.__text_bio_data[data_name]

    def add_file(self, file_path, data_name, if_train_file):
        text, slot, intent = self.__read_file(file_path)
        bio = []
        for s in slot:
            if self.__args.bio_schame == 'bio':
                bio_item = [i[0] for i in s]
                bio.append(bio_item)
            elif self.__args.bio_schame == 'bmeso':
                bio_item = [i[0] for i in bio2bmes(s)]
                bio.append(bio_item)
            else:
                raise NotImplementedError()
        
        if if_train_file:
            if not self.__args.use_bert:
                self.__word_alphabet.add_instance(text)
            else:
                self.__word_alphabet.add_instance
            self.__slot_alphabet.add_instance(slot)
            self.__intent_alphabet.add_instance(intent)
            self.__bio_alphabet.add_instance(bio)

        # Record the raw text of dataset.
        self.__text_word_data[data_name] = text
        self.__text_slot_data[data_name] = slot
        self.__text_intent_data[data_name] = intent
        self.__text_bio_data[data_name] = bio

        # Serialize raw text and stored it.
        self.__digit_word_data[data_name] = self.__word_alphabet.get_index(
            text)
        if if_train_file:
            self.__digit_slot_data[data_name] = self.__slot_alphabet.get_index(
                slot)
            self.__digit_intent_data[data_name] = self.__intent_alphabet.get_index(
                intent)
            self.__digit_bio_data[data_name] = self.__bio_alphabet.get_index(
                bio)
