...
from utils.data_utils import get_chunks

class ModelManager(nn.Module):

    def __init__(self, args, num_word, num_slot, num_intent, num_bio, bio_alphabet):
        super(ModelManager, self).__init__()

        self.__num_word = num_word
        self.__num_slot = num_slot
        self.__num_intent = num_intent
        self.__num_bio = num_bio
        self.__args = args
        self.bio_alphabet = bio_alphabet
        ...
        self.extra_template_dim = self.__args.encoder_hidden_dim * 2 if self.__args.template_encoder else  self.__args.word_embedding_dim
        self.__slot_decoder = LSTMDecoder(
            ...
            extra_dim=self.__num_intent +self.__num_bio +  self.extra_template_dim,
        )

        self.__bio_decoder = LSTMDecoder(
            self.__args.encoder_hidden_dim + self.__args.attention_output_dim,
            self.__args.bio_decoder_hidden_dim,
            self.__num_bio, self.__args.dropout_rate,
            embedding_dim=self.__args.bio_embedding_dim
        )
        

        if self.__args.template_encoder:
            self.__dropout_layer = nn.Dropout(self.__args.dropout_rate)
            if not self.__args.template_share_encoder:
                self.__template_encoder =  nn.LSTM(
                    input_size=self.__args.word_embedding_dim,
                ...
                self.__template_encoder = self.__encoder.__lstm_layer

        #Adaptive Tempalte Regulation
        # self.__entity_tensor = nn.Parameter(
        ...
        self.__bio_embedding = nn.Embedding(
            self.__num_bio, self.__num_bio
        )
        self.__bio_embedding.weight.data = torch.eye(self.__num_bio)
        self.__bio_embedding.weight.requires_grad = False                                                                         
    ...
    def forward(self, text, seq_lens, n_predicts=None, forced_slot=None, forced_intent=None, force_bio=None):
        word_tensor, _ = self.__embedding(text)
        device = word_tensor.device
        flat_text  = torch.cat([text[i][:seq_lens[i]] for i in range(0, len(seq_lens))], dim=0)
        ...
        pred_bio = self.__bio_decoder(
            hiddens, seq_lens, forced_input=force_bio
        )

        if not self.__args.differentiable:
            _, idx_bio = pred_bio.topk(1, dim=-1)
            feed_bio = self.__bio_embedding(idx_bio.squeeze(1))
        else:
            feed_bio = pred_bio
        ...
        for sent_i in range(0, len(seq_lens)):
            ...
            bio_chunks = idx_bio[sent_start_pos:sent_end_pos]
            word_list = flat_text[sent_start_pos:sent_end_pos].cpu().data.numpy().tolist()
            bio_instance = self.bio_alphabet.get_instance(bio_chunks)
            chunks = get_chunks(['O'] + bio_instance + ["O"])
            for chunk in chunks:
                s , e , _ = chunk
                for i in range(s-1, e):
                    word_list[i] = 2
            assert len(word_list) == seq_lens[sent_i]
            template_list.extend(word_list)
            sent_start_pos = sent_end_pos
        
        template_tensor = torch.tensor(template_list, device = device)
        template_word_tensor, _ = self.__embedding(template_tensor)
        sent_start_pos = 0
        output_tensor_list = []
        if self.__args.template_encoder:
            for sent_i in range(0, len(seq_lens)):
                sent_end_pos = sent_start_pos + seq_lens[sent_i]
                combined_input = template_word_tensor[sent_start_pos:sent_end_pos]
                dropout_input = self.__dropout_layer(combined_input)
                lstm_out, _ = self.__template_encoder(
                        dropout_input.view(1, seq_lens[sent_i], -1))
                output_tensor_list.append(lstm_out.view(seq_lens[sent_i], -1))
                sent_start_pos = sent_end_pos
            template_word_tensor = torch.cat(output_tensor_list, dim=0)
    
        extra_input = torch.cat((feed_intent, feed_bio, template_word_tensor), dim=1)
        ...
        if n_predicts is None:
            return F.log_softmax(pred_slot, dim=1), F.log_softmax(pred_intent, dim=1), F.log_softmax(pred_bio, dim=1)
        else:
            _, slot_index = pred_slot.topk(n_predicts, dim=1)
            _, intent_index = pred_intent.topk(n_predicts, dim=1)
            _, bio_index = pred_bio.topk(n_predicts, dim=1)
            return slot_index.cpu().data.numpy().tolist(), intent_index.cpu().data.numpy().tolist(), bio_index.cpu().data.numpy().tolist()
...
class LSTMEncoder(nn.Module):
    ...
    def forward(self, embedded_text, seq_lens):
        ...
        packed_text = pack_padded_sequence(
            dropout_text, seq_lens, batch_first=True)
        ...
        if self.__embedding_dim is None or forced_input is not None:
            ...
            for sent_i in range(0, len(seq_lens)):
                ...
                if self.__embedding_dim is not None and forced_input is not None:
                    ...
                    combined_input = torch.cat(
                        [seg_hiddens, seg_prev_tensor], dim=1)
                else:
            ...
            for sent_i in range(0, len(seq_lens)):
                ...
                for word_i in range(sent_start_pos, sent_end_pos):
                    ...
                    dropout_input = self.__dropout_layer(
                        combined_input).view(1, 1, -1)
